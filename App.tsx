...
import AnggaranDipaPage from './pages/AnggaranDipaPage';
import SakipRbPage from './pages/SakipRbPage';
import DisiplinLhkpnPage from './pages/DisiplinLhkpnPage';
...
import LayananSDMPage from './pages/LayananSDM/LayananSDMPage';
import PengajuanLayananPage from './pages/LayananSDM/PengajuanLayananPage';
import PengajuanSayaPage from './pages/LayananSDM/PengajuanSayaPage';
import DetailPengajuanPage from './pages/LayananSDM/DetailPengajuanPage';
    ...
    >
      ...
      {!collapsed && (
        <div className="ml-4 flex-1 flex items-center justify-between overflow-hidden">
          <span className="font-bold text-[11px] tracking-[0.1em] whitespace-nowrap overflow-hidden truncate">
            {label}
          </span>
          {badge !== undefined && badge !== null && (
            <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black text-white ${badgeColor} ${badgePulse ? 'animate-pulse' : ''}`}>
              {badge}
            </span>
          )}
        </div>
...
const AppContent = () => {
  ...
  useEffect(() => {
    const computeSdmBadges = () => {
      try {
        const raw = localStorage.getItem('layanan_sdm_pengajuan_db');
        if (!raw) return;
        const list: PengajuanSDM[] = JSON.parse(raw);
        if (!Array.isArray(list)) return;

        // User badges
        if (user?.nip) {
          const myTickets = list.filter(p => p.nip === user.nip);
          const needRevision = myTickets.filter(p => p.status === 'PERLU_PERBAIKAN').length;
          const inProgress = myTickets.filter(p => p.status === 'DIAJUKAN' || p.status === 'DALAM_PROSES' || p.status === 'DIVERIFIKASI').length;
          if (needRevision > 0) {
            setUserSdmBadge({ count: needRevision, isUrgent: true });
          } else if (inProgress > 0) {
            setUserSdmBadge({ count: inProgress, isUrgent: false });
          } else {
            setUserSdmBadge({ count: 0, isUrgent: false });
          }
        }

        // Admin badges
        const pendingAdmin = list.filter(p => p.status === 'DIAJUKAN').length;
        setAdminSdmBadge(pendingAdmin);
      } catch (e) {
        // silent
      }
    };
    ...
    computeSdmBadges();
    window.addEventListener('storage_updated', computeSdmBadges);
    window.addEventListener('sdm_notifications_updated', computeSdmBadges);
    return () => {
      window.removeEventListener('storage_updated', computeSdmBadges);
      window.removeEventListener('sdm_notifications_updated', computeSdmBadges);
    };
  }, [user]);
  ...
  const hasAccess = (path: string) => {
    ...
    if (['/settings', '/logs'].includes(path)) return isSuperadmin || hasRole('Superadmin');

    // Strict Admin/Editor-only pages
    if (['/admin/layanan-sdm', '/ukom/admin', '/ukom/supervisor', '/admin/attendance', '/admin/attendance/locations', '/admin/attendance/settings'].includes(path)) {
      return canEdit || isSuperadmin || hasRole('Superadmin') || hasRole('Admin Perencanaan & Layanan') || hasRole('Admin Pengembangan Kompetensi') || hasRole('Admin Pengelolaan Karier');
    }

    if (['/persuratan', '/tugas-rutin', '/kegiatan', '/laporan', '/keuangan', '/dossiers'].includes(path)) {
      if (canEdit || isSuperadmin || hasRole('Superadmin') || hasRole('Editor') || hasRole('Admin Perencanaan & Layanan')) return true;
    }
    ...
    if (employeeSelfServicePaths.includes(path) || path.startsWith('/pegawai/')) {
      ...
      const roleMatch = access.roles.some(r => hasRole(r));
      const userNip = (user?.nip || '').trim();
      const nipMatch = (access.nips || []).some(n => n.trim() === userNip);
      const isViewerOrUser = hasRole('Viewer') || hasRole('User') || hasRole('Pegawai');
  ...
  const MaintenanceView = () => (
    <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
      ...
      <Link to={user?.role === 'Admin Uang Makan' ? "/uang-makan" : "/"} className="mt-10 px-8 py-4 bg-blue-600 text-white rounded-2xl font-bold tracking-widest shadow-xl hover:bg-blue-700 transition-all">Kembali</Link>
    </div>
  );
  ...
  const AccessDeniedView = () => (
    <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
      ...
      <Link to={user?.role === 'Admin Uang Makan' ? "/uang-makan" : "/"} className="mt-10 px-8 py-4 bg-gray-900 text-white rounded-2xl font-bold tracking-widest shadow-xl hover:bg-gray-800 transition-all">Kembali</Link>
    </div>
  );
        ...
        <header className="bg-white border-b border-gray-100 shrink-0 z-[100] no-print">
          <div className="h-16 md:h-20 flex items-center justify-between px-4 md:px-8">
            ...
            <div className="flex items-center gap-2 md:gap-4">
              {/* Role Switcher Pill for Multi-Role Admins */}
              {userRoles && userRoles.length > 1 && (
                <div className="relative group">
                  <div className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50/80 border border-blue-200 rounded-xl cursor-pointer hover:bg-blue-100 transition-all">
                    <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></span>
                    <span className="text-[9px] font-black uppercase text-blue-700 max-w-[130px] truncate">
                      {activeRole || user?.role || 'Peran'}
                    </span>
                    <i className="bi bi-chevron-down text-[8px] text-blue-500"></i>
                  </div>
                  {/* Dropdown Menu */}
                  <div className="absolute right-0 top-full mt-1.5 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 hidden group-hover:block z-[200] animate-fadeIn">
                    <p className="text-[8px] font-black uppercase text-gray-400 px-3 py-1.5 tracking-wider">Pilih Peran Aktif ({userRoles.length} Role)</p>
                    <div className="space-y-1">
                      {userRoles.map((r) => {
                        const isActive = (activeRole || user?.role) === r;
                        return (
                          <button
                            key={r}
                            onClick={() => setActiveRole(r)}
                            className={`w-full text-left px-3 py-2 rounded-xl text-[10px] font-black uppercase flex items-center justify-between transition-all ${
                              isActive ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-gray-50'
                            }`}
                          >
                            <span className="truncate">{r}</span>
                            {isActive && <i className="bi bi-check-lg"></i>}
                          </button>
                        );
                      })}
                    </div>

...
const Dashboard = () => {
  ...
  const pendidikanData = useMemo(() => {
    const counts: Record<string, number> = { 'S3': 0, 'S2': 0, 'S1/D4': 0, 'D3': 0, 'SLTA': 0, 'SD': 0, 'LAINNYA': 0 };
    pegawai.forEach(p => {
      const edu = (p.pendidikan || '').toUpperCase().replace(/[\s\-\/]/g, '');
      
      if (edu.includes('S3') || edu.includes('DOKTOR')) {
        counts['S3']++;
      } else if (edu.includes('S2') || edu.includes('MAGISTER') || edu.includes('MASTER') || edu.startsWith('M.')) {
        counts['S2']++;
      } else if (edu.includes('S1') || edu.includes('DIV') || edu.includes('D4') || edu.includes('SARJANA')) {
        counts['S1/D4']++;
      } else if (edu.includes('DIII') || edu.includes('D3')) {
        counts['D3']++;
      } else if (edu.includes('SLTA') || edu.includes('SMA') || edu.includes('SMK') || edu.includes('SMEA') || edu.includes('STM')) {
        counts['SLTA']++;
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
         <div className="lg:col-span-5 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col h-[450px]">
            <h4 className="text-[10px] font-black text-gray-900 uppercase tracking-widest mb-6">Demografi Gender</h4>
            <div className="flex-1 relative min-h-0">
               <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={genderData} cx="50%" cy="45%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
         ...
         <div className="lg:col-span-7 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col h-[450px]">
            <h4 className="text-[10px] font-black text-gray-900 uppercase tracking-widest mb-6">Tingkat Pendidikan (Terkonsolidasi)</h4>
            <div className="flex-1 relative min-h-0">
               <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={pendidikanData} layout="vertical" margin={{ left: 20, right: 30, top: 10, bottom: 10 }}>

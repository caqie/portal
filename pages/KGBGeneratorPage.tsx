...
const KGBGeneratorPage = () => {
  ...
  const inputNoCapsClass = "w-full px-5 py-3.5 bg-gray-50 border-2 border-gray-100 rounded-2xl text-[12px] font-black outline-none focus:border-blue-600 focus:bg-white transition-all text-gray-950";
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 no-print">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Otomatisasi Naskah Dinas KGB ASN DJKI</p>
          </div>
        </div>
                 ...
                 {kgbHistory.map(k => (
                   <tr key={k.id} className="hover:bg-blue-50/5 group transition-colors">
                      <td className="px-10 py-5">
                         ...
                         <p className="text-[9px] font-mono text-blue-600">NIP. {k.nip}</p>
                      </td>
                      <td className="px-4 py-5">
                         <p className="text-[10px] font-bold text-gray-500 uppercase">{k.nomorSk || '-'}</p>
                         <p className="text-[11px] font-black text-gray-900 uppercase">TMT: {k.tmtBaru}</p>
                      </td>
                      <td className="px-4 py-5 text-center">
                         <span className="px-4 py-1.5 bg-emerald-50 text-emerald-600 rounded-xl text-[10px] font-black border border-emerald-100">Rp {Number(k.gajiBaru).toLocaleString('id-ID')}</span>
                      </td>
                      <td className="px-10 py-5 text-right">
                         <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                            ...
                            {canEdit && (
                              <>
                                <button onClick={() => handleEditKgb(k)} className="h-9 w-9 bg-white border border-gray-100 text-amber-500 rounded-xl flex items-center justify-center hover:bg-amber-50 shadow-sm"><i className="bi bi-pencil-fill"></i></button>
                                {isSuperadmin && <button onClick={() => { setItemToDelete(k); setIsConfirmOpen(true); }} className="h-9 w-9 bg-white border border-gray-100 text-rose-500 rounded-xl flex items-center justify-center hover:bg-rose-50 shadow-sm"><i className="bi bi-trash-fill"></i></button>}
                              </>
                            )}
                         </div>
                      </td>
        ...
        <div className="max-w-5xl mx-auto bg-white p-10 md:p-14 rounded-[4rem] border border-gray-100 shadow-sm space-y-12 animate-modalEnter">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                 <h5 className="text-[10px] font-black text-blue-600 uppercase border-b pb-2 tracking-widest">1. Atribut Surat</h5>
                 ...
                 <FormItem label="Nama Lengkap (Database)"><input type="text" className={inputNoCapsClass} value={formData.namaPegawai} onChange={e=>setFormData({...formData, namaPegawai: e.target.value})} /></FormItem>
              ...
              <div className="space-y-6">
                 <h5 className="text-[10px] font-black text-emerald-600 uppercase border-b pb-2 tracking-widest">2. Rincian Gaji & TMT</h5>
                 <div className="grid grid-cols-2 gap-4">
                 ...
                 <h5 className="text-[10px] font-black text-indigo-600 uppercase border-b pb-2 tracking-widest mt-4">3. Penandatangan</h5>
           ...
           <div className="pt-10 border-t flex justify-center">
              <button onClick={handleSave} disabled={syncing} className="px-24 py-5 bg-blue-600 text-white rounded-[2rem] font-black uppercase text-[10px] tracking-widest shadow-2xl active:scale-95 transition-all">Generate & Pratinjau F4</button>
           </div>
        </div>
      )}
           ...
           <div className="bg-gray-300 py-10 flex justify-center overflow-x-auto no-scrollbar">
              <div ref={pdfRef} className="bg-white shadow-2xl p-[1.5cm_2.2cm] font-arial text-black" style={{ width: '210mm', minHeight: '330mm' }}>
                 ...
                 <div className="text-center mb-10">
                    ...
                    <p className="text-[11pt] font-bold mt-1 uppercase">NOMOR : {formData.nomor}</p>
                 </div>
                 <div className="text-[11pt] space-y-6 text-justify leading-relaxed">
                    ...
                    <div className="grid grid-cols-[180px_10px_1fr] ml-10 gap-y-1">
                       ...
                       <span>3. Pangkat/Gol</span><span>:</span><span className="uppercase">{pSubjek?.pangkat || '-'} ({formData.golongan})</span>
                       <span>4. Jabatan</span><span>:</span><span className="uppercase">{formData.jabatan}</span>
                       <span>5. Unit Kerja</span><span>:</span><span className="uppercase">{normalizeUnitName(formData.unitKerja)}</span>
                    </div>
                    <p>Diberikan kenaikan gaji berkala, sehingga kepadanya diberikan gaji pokok baru sebesar:</p>
                    <div className="bg-gray-100 p-6 border-2 border-black rounded-xl text-center">
                       <p className="text-2xl font-black">Rp {Number(formData.gajiBaru).toLocaleString('id-ID')},-</p>
                       <p className="text-[9pt] italic mt-1">(Gaji pokok lama: Rp {Number(formData.gajiLama).toLocaleString('id-ID')},-)</p>
                    </div>
                 ...
                 <div className="mt-20 ml-[55%] text-center text-[11pt] leading-tight">
                    <p>Jakarta, {new Date(formData.tglSurat).toLocaleDateString('id-ID', {day:'numeric', month:'long', year:'numeric'})}</p>
                    <p className="font-bold uppercase mb-28 mt-2">{formData.pjbJabatan},</p>
